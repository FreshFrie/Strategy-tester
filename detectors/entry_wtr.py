...
def detect_wtr(arr, day_ctx, params):
    ...
    omega = float(params.get("omega", 0.45))
    tau = float(params.get("tau", 0.55))
            ...
            if lo[s] < pre_lo - omega * atr[s] and cl[s] > pre_lo:
                ...
                if inward >= tau and s + 1 < l1:
                    ...
                    if cl[c] > (op[s] + 0.25 * (cl[s] - op[s])) and cl[c] > hi[c - 1]:
                        out.append({"entry_idx": c, "direction": "LONG", "stop_price": float(lo[s] - 0.1 * atr[s]), "note": "wtr", "features": {"wick_frac": float((cl[s]-lo[s])/(hi[s]-lo[s]) if (hi[s]-lo[s])>0 else 0.0), "inward_close": float((cl[s]-lo[s])/(hi[s]-lo[s]) if (hi[s]-lo[s])>0 else 0.0), "confirm": True}, "score": None})
        else:
            if hi[s] > pre_hi + omega * atr[s] and cl[s] < pre_hi:
                ...
                if inward >= tau and s + 1 < l1:
                    ...
                    if cl[c] < (op[s] - 0.25 * (op[s] - cl[s])) and cl[c] < lo[c - 1]:
                        out.append({"entry_idx": c, "direction": "SHORT", "stop_price": float(hi[s] + 0.1 * atr[s]), "note": "wtr", "features": {"wick_frac": float((hi[s]-cl[s])/(hi[s]-lo[s]) if (hi[s]-lo[s])>0 else 0.0), "inward_close": float((hi[s]-cl[s])/(hi[s]-lo[s]) if (hi[s]-lo[s])>0 else 0.0), "confirm": True}, "score": None})
    return out
